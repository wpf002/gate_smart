...
def _ab_sentence(ab: dict) -> tuple[str, str]:
    """Plain-English read on the lesson experiment: (headline, what it means)."""
    from scripts.score_lessons import two_proportion_p

    m_w = round(ab["measured_rate"] * ab["measured_n"])
    r_w = round(ab["recency_rate"] * ab["recency_n"])
    p = two_proportion_p(m_w, ab["measured_n"], r_w, ab["recency_n"])
    ahead = "ahead" if ab["diff"] > 0 else "behind"
    headline = (
        f"Picking lessons by track record: {ab['measured_rate']:.1%} winners "
        f"over {ab['measured_n']} races. Picking the newest ones instead: "
        f"{ab['recency_rate']:.1%} over {ab['recency_n']}."
    )
    if p is not None and p < 0.05:
        meaning = (
            f"That is a real gap — {abs(ab['diff']):.1f} points {ahead}, and big "
            f"enough now that luck does not explain it."
        )
    else:
        meaning = (
            f"{abs(ab['diff']):.1f} points {ahead}, but that is still close enough "
            f"to be luck. Treat the two as tied until the gap holds over more races."
        )
    return headline, meaning


def _playbook_sentences(status: dict) -> list[str]:
    """The playbook in sentences rather than a row of labelled counts.

    The counts alone read as jargon — "0 proven · 8 awaiting evidence" tells you
...
def _render_playbook_html(status: dict) -> str:
    if not status or not status.get("active"):
        return ""
    body = " ".join(_playbook_sentences(status))
    ab = status.get("ab")
    if ab:
        headline, meaning = _ab_sentence(ab)
        experiment = (
            f'<p style="font-size:13px;color:#444;margin:10px 0 4px">'
            f'<b>Two ways of choosing those lessons are being raced against each '
            f'other.</b> {headline}</p>'
            f'<p style="font-size:13px;color:#777;margin:4px 0">{meaning}</p>'
        )
    else:
        experiment = (
            '<p style="font-size:13px;color:#777;margin:10px 0 4px">'
            'A second way of choosing lessons is being tested, but not enough '
            'races have finished to compare them yet.</p>'
        )
    return f"""
  <h2 style="color:#4a6a8a">📚 Playbook</h2>
  <p style="font-size:13px;color:#444;margin:4px 0">{body}</p>
{experiment}"""
...
def _render_playbook_text(status: dict) -> str:
    if not status or not status.get("active"):
        return ""
    lines = ["", "PLAYBOOK"]
    for sentence in _playbook_sentences(status):
        lines.append(f"  {sentence}")
    ab = status.get("ab")
    if ab:
        headline, meaning = _ab_sentence(ab)
        lines += ["",
                  "  Two ways of choosing those lessons are being raced against each other.",
                  f"  {headline}", f"  {meaning}"]
    else:
        lines += ["", "  A second way of choosing lessons is being tested, but not enough",
                  "  races have finished to compare them yet."]
    return "\n".join(lines) + "\n"
